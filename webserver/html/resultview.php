...
function loadData(changes){
	
	console.log(objectLimit+" loadData called with timestamp:"+timestamp);
	

	$.getJSON('getData.php',{tablename:dataset,timestamp:timestamp}, function(data){
        if(data.length==0){
			//No data found
		...
		$.each(data, function(key, val) {
			if(key==0){
				...
				//If that ID is already here , update it
				if((obj = search(val.ID))!=null){
					obj["tweets"] = val.mag;
					changes = true;
				} else {
					var obj = new Object();
